...
$(document).ready(function () {
  ...
  if (searchMap.id) {
    graph.id = searchMap['id'];
    showGraph(searchMap.id);
  }
  $("#modal-save").click(function () {
    graph.name = $("#modal-name").val();
    graph.requestType = $("#modal-type").val();
    graph.termLanguage = $("#modal-term-language").val();
    graph.dataClusters = $("#modal-data-clusters").val();
    save = true;
  ...
  $.ajax({
    ...
    success: function(data) {
      if (data.success && data.data) {
        console.log(data);
        $("#showXMLiframe").attr("src", "api/v1/bigraph/xml?id=" + graph.id);
        // 设置页面数据
        $("#modal-name").val(data.data.graph.name ? data.data.graph.name : '');
        $("#modal-type").val(data.data.graph.requestType ? data.data.graph.requestType : '');
        $("#modal-term-language").val(data.data.graph.termLanguage ? data.data.graph.termLanguage : '');
        $("#modal-data-clusters").val(data.data.graph.dataClusters ? data.data.graph.dataClusters : '');
        if (data.data.linkList && data.data.linkList.length > 0) {
          $("#link tr:not(:first)").remove();
        }
        data.data.linkList.forEach(element => {
          element.linkFrom = element.linkFrom ? element.linkFrom : "";
          element.linkTo = element.linkTo ? element.linkTo : "";
          var node = '<tr>' + 
          "<input  style= width:100px; type=text size=10>"+ element.linkFrom  +"</input>" +
          "<input  style= width:100px; type=text size=10>"+ element.linkTo +"</input>" +
          '</tr>';
        });
        if (data.data.portList && data.data.portList > 0) {
          $("#port tr:not(:first)").remove();
        }
        data.data.portList.forEach(element => {
          element.portId = element.portId ? element.portId : '';
          element.portName = element.portName ? element.portName : '';
          element.dataType = element.dataType ? element.dataType : '';
          element.portType = element.portType ? element.portType : '';
          element.portType = element.portType ? element.portType : null;
          var node = '<tr>' +
          "<input  style= width:100px; type=text size=10>"+ element.portId  +"</input>" +
          "<input  style= width:100px; type=text size=10>"+ element.portName +"</input>" +
          "<input  style= width:100px; type=text size=10>"+ element.dataType +"</input>"+
          "<select style= width:100px;  id = Ctrl><option value ="+element.portType+">input</option><option value =output>output</option> <option value=callput>callput</option></select>" +
          "<select  style= width:100px; id = cL><option value ="+element.portType+"></option><option value =outerlink>outerlink</option><option value=innerlink>innerlink</option></select>";
          '</tr>';
          $("#port tbody").append(node);
        });
        if (data.data.serviceList && data.data.serviceList.length > 0) {
          $("#Service tr:not(:first)").remove();
        }
        data.data.serviceList.forEach(element => {
          var node = '<tr>' + 
          "<td><input  style= width:100px; type=text size=8>"+ element.serviceId +"</input></td>" +
          "<td><input   style= width:100px; type=text size=8>"+ element.serviceName +"</input></td>" +
          "<td><input  style= width:100px; type=text size=8>"+ element.serviceParentNode +"</input></td>" +
          "<td><input  style= width:100px; type=text size=8>"+ element.serviceChildrenCount +"</input></td>" +
          "<td><input  style= width:100px; type=text size=8>"+ element.serviceInner +"</input></td>" +
          "<td><input  style= width:100px; type=text size=8>"+ element.serviceSiteNumber +"</input></td>" +
          '</tr>';
          $("#Service tbody").append(node);
        });
        if (data.data.siteList && data.data.siteList.length > 0) {
          $("#root tr:not(:first)").remove();
        }
        data.data.siteList.forEach(element => {
          console.log("add site input" + element)
          var newLine = edge.rows.length;
          // var row = edge.insertRow(newLine);
          // var col = row.insertCell(0);
          // col = row.insertCell(0);
          // col.innerHTML = "<input  style= width:100px; type=text size=8>"+ element.siteId ? element.siteId : '' +"</input>";;
          // col = row.insertCell(1);
          // col.innerHTML = "<input style= width:100px;  type=text size=8>"+ element.siteOuterId ? element.siteOuterId : '' +"</input>";;
          element.siteId = element.siteId ? element.siteId : "";
          element.siteOuterId = element.siteOuterId ? element.siteOuterId : "";
          var node = '<tr><td><input style="width:100px;" name="text" type="text" size="8" value="'+ element.siteId +'"></td><td>' +
          '<input style="width:100px;" type="text" size="8" value="' + element.siteOuterId + '"></td></tr>';
          // var nodeA = '<tr><td><input style="width:100px;" name="text" type="text" size="8" value="'+ element.siteId +'"></td></tr>';
          // console.log(nodeA);
          $("#root tbody").append(node)
        });
      }
    }
  })
...
function addLine3() {
  var newLine = root.rows.length;
  var row = root.insertRow(newLine);
  var col = row.insertCell(0);
  col = row.insertCell(0);
  col.innerHTML = "<input  style= width:100px; type=text size=8>";
  col = row.insertCell(1);
...
function delLine() {
  var i;
  var count;
  count = port.rows.length;
  if (count > 2) {
    port.deleteRow(port.rows.length - 1)
    for (i = 0; i < count - 1; i++) {
...
function draw() {
  ...
  //alert("check1");
  var Dc = new DC();
  Dc.S_N = (tagElementSi.length) / 6; //服务数量
  //alert(Dc.S_N);
  Dc.RootNumber = (tagElementL.length) / 2;
  //alert(Dc.RootNumber);
  var s = new Array();
  var cir = new Array();
  var root = new Array();
  ...
  for (var i = 0; i < Dc.RootNumber; i++) {
    root[i] = new Root();
    if (tagElementL[i * 2 + 1].value != "") {
      root[i].osite = true;
      siteList[i] = {};
      siteList[i].siteId = tagElementL[i * 2].value;
      root[i].ositename = tagElementL[i * 2 + 1].value;
      siteList[i].siteOuterId = tagElementL[i * 2 + 1].value;
  ...
  for (var i = 1; i <= Dc.S_N; i++) {
    var j = (i - 1) * 6;
    //alert(tagElementSi[j].value );
    if (tagElementSi[j].value == "") break;
    s[i].name = tagElementSi[j].value;
    serviceList[i] = {};
    serviceList[i].name = tagElementSi[j].value;
    if (tagElementSi[j + 5].value != "") {
      s[i].root_number = tagElementSi[j + 5].value;
      serviceList[i].serviceSiteNumber = tagElementSi[j + 5].value;
    }
    if (tagElementSi[j + 2].value == "") {
      root[s[i].root_number].Nofacir++;
    } else {
      s[i].fatherNode = tagElementSi[j + 2].value;
      serviceList[i].serviceParentNode = tagElementSi[j + 2].value;
      s[i].root_number = s[get_num(s[i].fatherNode)].root_number;
    }
    if (tagElementSi[j + 3].value != "") {
      s[i].inserve = tagElementSi[j + 3].value;
      serviceList[i].serviceChildrenCount = tagElementSi[j + 3].value;
    }
    if (tagElementSi[j + 4].value != "") {
      s[i].site = true;
      s[i].sitename = tagElementSi[j + 4].value;
      serviceList[i].serviceInner = tagElementSi[j + 4].value
      Dc.sitenumber++;
    }
    //alert(s[i].name);
  ...
  for (var i = 1; i <= portN; i++) {
    for (var j = 1; j <= Dc.S_N; j++) {
      if (s[j].name == gets(tagElementPi[(i - 1) * 3].value)) {
        ...
        portList[j].portType = tagElementPc[(i - 1) * 2].options[index].value;
  ...
  for (var i = 0; i < Dc.E_N; i++) {
    ...
    linkList[i].linkFrom = tagElementLi[(i * 2)].value;
    link[i].end = tagElementLi[(i * 2 + 1)].value;
    linkList[i].linkTo = tagElementLi[(i * 2 + 1)].value;
  }
    ...
    if (graph.id) {
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.success) {
            alert("insert success!");
            $("#showXMLiframe").attr("src", "api/v1/bigraph/xml?id=" + data.data.graph.id);
            // 保存id，使得下次就是更新
            graph.id = data.data.graph.id
          }
        }
      })
    }
  ...
  for (var i = 0; i < Dc.RootNumber; i++) {
    ...
    //多个服务内嵌套多个服务未写;
      ...
      var Li = (l - 40) / root[i].Nofacir;
      var Wi = (w - 40) / root[i].Nofacir;
      var xi = (x + 20 + Li + x) / 2;
      var yi = (y + 20 + Wi + y) / 2;
      for (var j = 1; j <= Dc.S_N; j++) {
        if (s[j].root_number == i && s[j].fatherNode == -1) {
